...
interface CheckoutPageProps {
    ...
    handleOrder: (deliveryDetails: { name: string, phone: string, address: string }) => void;
...
const CheckoutPage: React.FC<CheckoutPageProps> = ({
    ...
    // Controlled inputs for delivery details
    const [deliveryName, setDeliveryName] = React.useState(user?.name || '');
    const [deliveryPhone, setDeliveryPhone] = React.useState(user?.phone || '');
    const [deliveryAddress, setDeliveryAddress] = React.useState(
        addresses.find(a => a.isDefault)?.fullAddress || addresses[0]?.fullAddress || ''
    );
